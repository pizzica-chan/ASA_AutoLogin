...
class LoginApp(tk.Tk):
    ...
    def _init_timing_vars(self) -> None:
        for key, _label, default, _vmin, _vmax, _inc, _group in RETRY_TIMING_FIELDS:
            if key == "start_countdown_seconds":
                self._timing_vars[key] = tk.IntVar(value=int(default))
            else:
                self._timing_vars[key] = tk.DoubleVar(value=default)
    ...
    def _build_ui(self) -> None:
        ...
        notebook.add(tab_timing, text="待機・タイミング")
    ...
    def _build_main_tab(self, parent: ttk.Frame) -> None:
        ...
        display_card = self._card(parent, "表示設定")
        monitor_row = ttk.Frame(display_card, style="Card.TFrame")
        monitor_row.pack(fill=tk.X, pady=3)
        ttk.Label(monitor_row, text="ARKモニター", style="Card.TLabel", width=16).pack(side=tk.LEFT)
        ...
        retry_card = self._card(parent, "リトライ設定")
        ...
        ttk.Label(retry_grid, text="失敗後待機 (秒)", style="Card.TLabel").grid(row=1, column=0, sticky=tk.W, pady=3)
        ...
        ttk.Label(retry_grid, text="詳細な待機は「待機・タイミング」タブ", style="CardDim.TLabel").grid(row=2, column=0, columnspan=3, sticky=tk.W, pady=(6, 0))

    def _build_timing_tab(self, parent: ttk.Frame) -> None:
        intro = ttk.Label(
            parent,
            text="ゲームの各場面でどれくらい待つかを秒単位で設定します。「設定を保存」で config.yaml に書き込みます。",
            style="Dim.TLabel",
            wraplength=600,
        )
        ...
        grid = ttk.Frame(timing_card, style="Card.TFrame")
        grid.pack(fill=tk.X)

        current_group = ""
        row = 0
        for key, label, default, vmin, vmax, increment, group in RETRY_TIMING_FIELDS:
            if group != current_group:
                ...
                tk.Label(
                    ...
                    bg=COLORS["surface"],
                    font=("Segoe UI", 10, "bold"),
                ).grid(row=row, column=0, columnspan=3, sticky=tk.W, pady=(10 if row else 0, 4))
                row += 1

            ttk.Label(grid, text=label, style="Card.TLabel", wraplength=280, justify=tk.LEFT).grid(
                row=row, column=0, sticky=tk.W, pady=3, padx=(0, 8),
            )
            var = self._timing_vars[key]
            if key == "start_countdown_seconds":
                widget = ttk.Spinbox(
                    grid,
                    from_=int(vmin),
            ...
            ttk.Label(grid, text=f"既定: {default:g}", style="CardDim.TLabel").grid(row=row, column=2, sticky=tk.W, padx=(8, 0))
            row += 1
    ...
    def _load_settings(self) -> None:
        ...
        for key, _label, default, _vmin, _vmax, _inc, _group in RETRY_TIMING_FIELDS:
            value = retry.get(key, default)
            if key == "start_countdown_seconds":
                self._timing_vars[key].set(int(value))
            else:
                self._timing_vars[key].set(float(value))

    def _collect_retry_timing(self) -> dict[str, float | int]:
        timing: dict[str, float | int] = {}
        for key in self._timing_vars:
            if key == "start_countdown_seconds":
                timing[key] = int(self._timing_vars[key].get())
            else:
                timing[key] = float(self._timing_vars[key].get())
        return timing
    ...
    def _run_worker(self) -> None:
        ...
        try:
            ...
            self._log_queue.put(("log", "自動ログインを開始します"))
